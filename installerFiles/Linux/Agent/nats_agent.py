...
async def handle_run_snippets_now(parameters: dict) -> dict:
    """Trigger the oneshot snippet runner via systemd without blocking NATS."""
    try:
        service_name = parameters.get('service', 'wegweiser-agent.service')
        ...
        try:
            ...
            return {"status": "error", "error": "systemctl start timed out"}
        ...
        logger.error(f"handle_run_snippets_now error: {e}")
...
async def main():
    ...
    try:
        ...
        if agent.nats:
            ...
            agent.nats.register_command_handler('run_snippets_now', handle_run_snippets_now)
