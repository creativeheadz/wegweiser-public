document.addEventListener("DOMContentLoaded", function () {
    ...
    function aggregateByDay(points) {
        ...
        return Object.entries(daily).map(([day, ys]) => ({
            x: new Date(day).getTime(),
            y: ys.reduce((a, b) => a + b, 0) / ys.length
        }));
    }

    fetch('/tenant/' + window.tenant_uuid + '/group_health_history')
        .then(r => r.json())
        .then(data => {
            ...
            const opts = {
                width: document.querySelector("#group-health-history").offsetWidth,
                height: 250,
            ...
            document.querySelector("#group-health-history").innerHTML = '';
            const uplot = new uPlot(opts, seriesData, document.querySelector("#group-health-history"));

            // Handle responsive resizing
            window.addEventListener("resize", () => {
                const width = document.querySelector("#group-health-history").offsetWidth;
