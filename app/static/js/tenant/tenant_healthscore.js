document.addEventListener("DOMContentLoaded", function () {
    const historyContainer = document.getElementById("tenant-health-history");
    ...
    const tenantId = historyContainer.dataset.tenantId;
    ...
    initTenantHealthHistory(tenantId);

    function initTenantHealthHistory(tenantId) {
        if (typeof uPlot === "undefined") {
            console.error("uPlot not loaded");
            return;
        }

        const container = document.getElementById("tenant-health-history");
        if (!container) return;
        ...
        fetch(`/tenant/${tenantId}/health_history`)
            .then(r => r.ok ? r.json() : [])
            .then(rows => {
                if (!rows || !rows.length) {
