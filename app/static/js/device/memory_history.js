    ...
    if ($("#memory-load-history").length) {
        ...
        $.get('/widgets/device/' + window.device_uuid + '/ram_history', function (response) {
            try {
                ...
                const containerHeight = $("#memory-load-history").parent().height();
                ...
                let containerWidth = $("#memory-load-history").width();
                ...
                const uplot = new uPlot(opts, data, document.querySelector("#memory-load-history"));

                $(window).on("resize", function () {
                    // Only resize if container is visible and has reasonable width
                    if ($("#memory-load-history")[0].offsetParent !== null) {
                        const newWidth = $("#memory-load-history").width();
                        if (newWidth >= 50) {
                            const newContainerHeight = $("#memory-load-history").parent().height();
                ...
                console.error("Error rendering memory load history chart:", error);
                $("#memory-load-history").html(
                    '<div class="alert alert-danger">Failed to render chart</div>'
                );
            }
        }).fail(function (jqXHR, textStatus, errorThrown) {
            console.error("Failed to load memory history data:", textStatus, errorThrown);
            $("#memory-load-history").html(
                '<div class="alert alert-danger">Failed to load memory history data</div>'
            );
        });
    }
});
