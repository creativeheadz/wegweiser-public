document.addEventListener('DOMContentLoaded', function () {
    const historyContainer = document.getElementById('group-health-history');
    ...
    const groupUuid = historyContainer.dataset.groupUuid;

    if (!groupUuid) {
        console.error("Invalid group UUID!");
        return;
    }

    // Initialize modern health history chart
    initGroupHealthHistory(groupUuid);

    function initGroupHealthHistory(groupUuid) {
        if (typeof uPlot === "undefined") {
            console.error("uPlot not loaded");
            return;
        }

        const container = document.getElementById("group-health-history");
        if (!container) return;
        ...
        fetch(`/groups/${groupUuid}/health_history`)
            .then(r => r.ok ? r.json() : [])
            .then(rows => {
                if (!rows || !rows.length) {
