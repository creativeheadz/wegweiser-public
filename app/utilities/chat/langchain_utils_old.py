...
def create_langchain_conversation(entity_uuid: str, entity_type: str, tenant_profile: str,
    ...
    base_prompt = """You are a technical assistant providing accurate information about devices.
...
"""
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are Wegweiser, an AI powered assistant for Managed Services Prviders (MSPs).
        ...
        When asking about health scores, status, or current metrics, always use force_refresh=True to get the most current data. Pleae, do not include any signature or final greeting. The messages are exchanged in an ongoing chat conversation so any such signature is awkward

        Use this format:
        Thought: Do I need to use a tool or check conversation history?
        Action: [get_device_info or reference history]
        Action Input: [query type or relevant history]
        Observation: [tool result or historical context]
        Thought: I have the information needed
        Answer: [Provide clear, detailed, and helpful response with context]
...
def generate_health_score_analysis(entity_type, entity_uuid):
    ...
    Your analysis should be thorough and actionable, providing clear steps for the MSP to take.
    """
...
def generate_tool_recommendations(tenant_uuid):
    ...
    Primary Focus: {tenant.primary_focus}
    Current Tools: {', '.join([f'{k}: {v}' for k, v in profile_data.items() if v != 'none'])}
    ...
    [Provide a brief overview of their current tools and how they fit together]

    ## Recommended Tools and Solutions

    [For each missing category, use this format:]

    ### [Category Name]

    **Current Status**: Not Implemented

    **Business Impact**: [Brief explanation of how this gap affects operations]

    **Recommendations**:
    - Top Enterprise Solution: [Name and brief description]
    - Mid-Range Option: [Name and brief description]
    - Budget-Friendly Choice: [Name and brief description]
    ...
    **Implementation Considerations**:
    ...
    [Continue for each major gap in their toolset]

    ## Implementation Priority
    [Suggest a prioritized order for implementing these tools based on their business impact and complexity]

    Keep the tone professional and consultative, focusing on business value and operational efficiency.
    """
