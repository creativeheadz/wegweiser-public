...
try:
	...
	print(f'Failed to import PIP. Reason: {e} Quitting.')
	quit()
import socket
import json
import os
...
def getDeviceUuid(configFile, args):
	...
	if os.path.isfile(configFile):
		try:
			...
			quit('Quitting.')
		groupUuid 	= configDict['groupuuid']
		deviceUuid 	= configDict['deviceuuid']
	else:
		logger.warning(f'{configFile} not found. Requesting...')
		if not args.groupUuid:
			logger.error('No groupUuid specified. Quitting')
			sys.exit()
		else:
			logger.debug(f'args.groupUuid: {args.groupUuid}')
			groupUuid = args.groupUuid
			logger.debug(f'groupUuid: {groupUuid}')
			deviceUuid 	= registerDevice(groupUuid)
			writeWegConfigFile(deviceUuid, groupUuid)
...
def getCollectorData(deviceDataDict):
	...
	deviceDataDict['collector']['collversion']		= appVersion
...
def runAndreiMsInfo():
	...
	msifoPsfile = os.path.join(appDir, 'Collector' , 'scripts', 'msinfo32-evaluator.ps1')
	logger.debug(f'Attempting to run powershell.exe -File {msifoPsfile}')
	command 	= ['powershell.exe', '-File', msifoPsfile]
...
#					logger.debug(f'msinfoline: {line}')
		...
		dmidecode_process 	= subprocess.Popen(command, stdout=subprocess.PIPE)
...
appName 		= 'wegweiserCollector'
appVersion		= '202407261120'
debugMode 		= True
...
wegConfigFile 	= f'{configDir}wegweiser.config'
deviceUuid, \
	groupUuid 	= getDeviceUuid(wegConfigFile, args)
...
if (mode.upper() == 'RESETEVENTS'):
	if platform.system() == 'Windows':
		removeEvents()
		sys.exit()
	else:
		logger.info('RESETEVENTS is not valid for this OS.')
		sys.exit()

###################### UPDATE ######################

if (mode.upper() == 'UPDATE') or (mode.upper() == 'FULLAUDIT') or (mode.upper() == 'AUDIT'):
	collectorUpdateReqd, \
	collHashPy, \
	collHashWin		= getServerCollVersion(appVersion)
	if collectorUpdateReqd == True:
		updateCollector(collHashPy, collHashWin)
		sys.exit('Exiting after update.')


###################### MSINFO ######################

if (mode.upper() == 'MSINFO') or (mode.upper() == 'FULLAUDIT') or (mode.upper() == 'INSTALL'):
	getMsInfo(msinfoOutput)


###################### AUDIT ######################

if (mode.upper() == 'AUDIT') or (mode.upper() == 'FULLAUDIT') or (mode.upper() == 'INSTALL'):
	deviceDataDict	= {}
...
if (mode.upper() == 'FULLAUDIT') or (mode.upper() == 'INSTALL'):
...
if (mode.upper() == 'AUDIT') or (mode.upper() == 'FULLAUDIT') or (mode.upper() == 'INSTALL'):
	...
	if debugMode == True:
		...
		with open(f'{logDir}{appName}.log', 'a') as f:
...
if (mode.upper() == 'EVENTLOG') or (mode.upper() == 'FULLAUDIT') or (mode.upper() == 'INSTALL'):
	route = '/payload/sendfile'
	if platform.system() == 'Windows':
		try:
			import win32evtlog
		...
		try:
			...
			pip.main(['install', 'win32evtlogutil'])
			import win32evtlogutil
...
elif mode.upper() == 'STATUS':
	route = '/payload/sendstatus'
	statusDict = {}

###################### WEGLOG ######################

elif mode.upper() == 'WEGLOG':
	route = '/payload/sendweglog'
	weglogDict = {}
