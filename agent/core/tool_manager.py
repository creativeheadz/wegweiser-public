...
class ToolManager:
    ...
    async def download_and_unpack_tool(self, tool_name: str) -> Optional[str]:
        ...
        try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(download_url) as response:
                        if response.status == 200:
                            ...
                            logger.error(f"Failed to download tool {tool_name}. Status: {response.status}")
                            return None
            else:
                logger.info("aiohttp not available, falling back to urllib for tool download")
                await loop.run_in_executor(None, self._download_with_urllib, download_url, local_path)
            ...
            logger.error(f"An error occurred while downloading/unpacking {tool_name}: {e}", exc_info=True)
            if os.path.exists(local_path):
                os.remove(local_path)
            return None

    def _extract_tar(self, tar_path: str, extract_path: str):
    ...
    async def run_tool(self, tool_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """
        Runs a tool with the given parameters. For Python tools, it ensures
        the correct virtual environment is used.
        """
        entry_point_path = await self.get_tool_executable(tool_name)
        if not entry_point_path:
            return {'status': 'error', 'message': f"Tool '{tool_name}' not available or failed to download."}

        # For python tools, we need to find the python executable in the agent's venv
        agent_venv_python = os.path.join(os.path.dirname(sys.executable), 'python.exe' if platform.system() == 'Windows' else 'python3')
        ...
        # This is a basic example, you might need more sophisticated parameter handling
        args = []
