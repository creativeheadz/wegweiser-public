...
def parseArgs():
	...
	parser.add_argument("-s", "--serverAddr",	help = "Enter Server Address",	required=False)	 
...
def registerDevice(groupuuid, agentPubPem, serverAddr):
	...
	response 		= sendJsonPayloadFlask(payload, endpoint, serverAddr)
	if debugMode == True:
...
def writeAgentConfigFile(deviceUuid, agentPrivPem, agentPubPem, serverPubPem, agentConfigFile, serverAddr):
	configDict = {
...
def checkAgentConfigFile():
	if not os.path.isfile(agentConfigFile):
		...
		if not args.groupUuid:
			...
			sys.exit()
		if not args.serverAddr:
			logger.error('serverAddr not specified with -s switch. Unable to register device.')
			sys.exit()		
		agentPrivPem, \
		...
		agentUuid 		= registerDevice(groupUuid, agentPubPem, serverAddr)
		writeSuccess 	= writeAgentConfigFile(agentUuid, agentPrivPem, agentPubPem, serverPubPem, agentConfigFile, serverAddr)
...
if not 'serverAddr' in agentConfigDict:
	host 			= 'app.wegweiser.tech'
else:
